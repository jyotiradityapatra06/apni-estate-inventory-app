  ...
  if (error || !data) {
    return (
      <div className="rounded-xl border border-dashed bg-white p-10 text-center">
        ...
        <p className="mt-1 text-sm text-slate-500">{error || "Purchase return not found."}</p>
    ...
    <div className="space-y-6 pb-12 max-w-4xl print:p-0 print:max-w-none">
      ...
      <div className="invoice-print-root rounded-xl border bg-white shadow-sm overflow-hidden print:border-none print:shadow-none">
        <div className="px-8 py-6 border-b bg-slate-50/50 flex justify-between items-start print:bg-white print:px-0">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">PURCHASE RETURN NOTE</h2>
        ...
        <div className="grid gap-6 md:grid-cols-2 p-8 print:px-0">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Supplier Details</h4>
            <div className="font-semibold text-slate-800">{data.supplier.name}</div>
            <div className="text-slate-600 text-sm mt-1">{data.supplier.phone}</div>
          </div>

          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Purchase Order References</h4>
            <div className="text-sm text-slate-800">
              Against Purchase Order:{" "}
              <Link to={`/purchases/${data.purchaseOrderId}`} className="font-semibold text-blue-700 hover:underline print:no-underline print:text-slate-800">
              ...
              {data.items.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-50/50">
                  <td className="px-8 py-4">
                    ...
                    <div className="text-xs text-slate-500">SKU: {item.inventoryItem.sku} · Source Godown: {item.godown.name}</div>
                  </td>
