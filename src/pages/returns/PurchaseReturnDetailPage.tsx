...
export default function PurchaseReturnDetailPage() {
  ...
  const canUpdate = hasPermission(user, "purchase-returns:update");
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Purchase return details could not be loaded.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePost = async () => {
    if (!window.confirm("Are you sure you want to complete this purchase return? This will deduct stock levels from the source godown and update supplier accounts, and cannot be undone.")) return;
    setActioning(true);
    try {
      await purchaseReturnApi.post(id!);
      toast.success("Purchase return completed successfully.");
      void load();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to complete purchase return.");
    } finally {
      setActioning(false);
    }
  };
  ...
  const handleCancel = async () => {
    ...
    setActioning(true);
    try {
      await purchaseReturnApi.cancel(id!, reason);
      toast.success("Purchase return cancelled successfully.");
      void load();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to cancel purchase return.");
    } finally {
      setActioning(false);
    }
  };
  ...
  if (error || !data) {
    return (
      <div className="rounded-xl border border-dashed bg-card p-10 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{error || "Purchase return not found."}</p>
        <button onClick={() => navigate("/purchase-returns")} className="mt-4 min-h-11 rounded-lg border px-4 font-semibold">
          Back to List
        </button>
      </div>
    );
    ...
    <div className="space-y-6 pb-12 max-w-4xl print:p-0 print:max-w-none">
      ...
      <div className="invoice-print-root rounded-xl border bg-card shadow-sm overflow-hidden print:border-none print:shadow-none">
        <div className="px-8 py-6 border-b bg-muted/50 flex justify-between items-start print:bg-card print:px-0">
          <div>
            <h2 className="text-2xl font-bold text-foreground">PURCHASE RETURN NOTE</h2>
        ...
        <div className="grid gap-6 md:grid-cols-2 p-8 print:px-0">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Supplier Details</h4>
            <div className="font-semibold text-foreground">{data.supplier.name}</div>
            <div className="text-muted-foreground text-sm mt-1">{data.supplier.phone}</div>
          </div>

          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Purchase Order References</h4>
            <div className="text-sm text-foreground">
              Against Purchase Order:{" "}
              <Link to={`/purchases/${data.purchaseOrderId}`} className="font-semibold text-blue-700 hover:underline print:no-underline print:text-foreground">
              ...
              {data.items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50">
                  <td className="px-8 py-4">
                    ...
                    <div className="text-xs text-muted-foreground">SKU: {item.inventoryItem.sku} · Source Godown: {item.godown.name}</div>
                  </td>
