...
export default function LedgerPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Total Credit (Cash In)</span>
            <ArrowDownLeft size={18} className="text-green-600" />
          </div>
          <strong className="text-xl sm:text-2xl font-black text-green-700 block">{fmt(totalCredit)}</strong>
          <span className="text-[10px] font-semibold text-muted-foreground block">Customer collections & receipts</span>
        </div>

        <div className="rounded-2xl border border-border bg-card p-5 shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Total Debit (Cash Out)</span>
            <ArrowUpRight size={18} className="text-red-600" />
          </div>
          <strong className="text-xl sm:text-2xl font-black text-red-600 block">{fmt(totalDebit)}</strong>
          <span className="text-[10px] font-semibold text-muted-foreground block">Vendor payments & expenses</span>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4 shadow-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <select
          ...
          className="min-h-[46px] rounded-xl border border-border bg-card px-4 text-sm font-bold text-foreground focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none cursor-pointer"
          value={party}
