...
export default function MorePage() {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl md:text-3xl font-bold text-foreground">More</h1><p className="mt-1 text-base text-muted-foreground">{business?.name || "Your business"}</p></div>
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
      {visible.map((item) => { const Icon = item.icon; return <button key={item.id} onClick={() => navigate(item.path)} className="min-h-28 rounded-xl border border-border bg-card p-4 text-left shadow-sm transition hover:border-blue-300 hover:shadow-md active:scale-[.98]">
