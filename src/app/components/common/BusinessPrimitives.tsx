...
export const CurrencyDisplay = ({ value }: { value: number | string | null | undefined }) => (
  <span className="tabular-nums">{fmt(Number(value || 0))}</span>
);

export const QuantityDisplay = ({ value, unit }: { value: number | string; unit?: string }) => (
  <span className="tabular-nums">{Number(value).toLocaleString("en-IN", { maximumFractionDigits: 3 })}{unit ? ` ${unit}` : ""}</span>
);
