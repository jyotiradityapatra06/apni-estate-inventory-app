import { LogOut, X, Bell, ChevronRight, Settings } from "lucide-react";
...
export function MobileMoreSheet({ open, onClose, onNotifications }: { open: boolean; onClose: () => void; onNotifications: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] md:hidden" role="dialog" aria-modal="true" aria-label="More menu">
      <button 
        className="absolute inset-0 bg-slate-950/60 backdrop-blur-xs cursor-default animate-fade-in" 
      ...
      <section className="absolute inset-x-0 bottom-0 flex max-h-[88dvh] flex-col rounded-t-3xl bg-white dark:bg-[#0F172A] shadow-2xl animate-in slide-in-from-bottom duration-250 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 p-4 shrink-0 space-y-2 pb-[max(16px,env(safe-area-inset-bottom))] bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <button 
            onClick={signOut} 
            className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-2xl bg-red-50 dark:bg-red-950/40 text-xs font-extrabold text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/60 cursor-pointer press-active border border-red-100 dark:border-red-900/50"
          >
            <LogOut size={16}/>
