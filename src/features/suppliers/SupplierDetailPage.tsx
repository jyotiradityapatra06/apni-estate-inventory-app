...
export function SupplierDetailPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <Link to="/suppliers" className="flex min-h-9 items-center gap-2 text-xs font-bold text-muted-foreground hover:text-orange-600 cursor-pointer">
        <ArrowLeft size={14} />
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2.5">
            <a
              href={`tel:${data.phone}`}
              className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 text-xs font-black text-muted-foreground shadow-xs hover:bg-muted cursor-pointer transition-colors"
            >
            ...
            <Link
              ...
              className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 text-xs font-black text-muted-foreground shadow-xs hover:bg-muted transition-colors cursor-pointer"
            >
            ...
            <Row l="Materials Supplied">
              <div className="flex flex-wrap gap-1.5 pt-1">
                {data.suppliedMaterials.map((m: any) => (
                  <span key={m.inventoryItem?.id || m.id} className="px-2.5 py-0.5 rounded-full bg-muted text-muted-foreground text-xs font-bold border border-border">
          ...
          {data.purchaseHistory.length ? (
            data.purchaseHistory.map((x) => (
              <Link
                key={x.id}
                to={`/purchases/${x.id}`}
                className="flex items-center justify-between rounded-xl border border-border/80 bg-muted/60 p-4 hover:bg-orange-50/50 hover:border-orange-200 transition-colors"
              >
                <div>
                  <b className="text-sm font-black text-foreground block">{x.purchaseOrderNumber}</b>
                  <span className="text-xs text-muted-foreground font-semibold mt-0.5 block">
                    {new Date(x.orderDate).toLocaleDateString("en-IN")}
                  </span>
                </div>
                <div className="text-right">
                  <b className="text-sm font-black text-foreground block">{fmt(x.totalAmount)}</b>
                  <BusinessStatusBadge status={x.status} />
                </div>
              </Link>
            ))
          ) : (
            <p className="text-xs font-semibold text-muted-foreground py-4 text-center">No purchase orders recorded yet.</p>
          )}
        </div>
      </section>
    </div>
...
function Row({ l, children }: { l: string; children: React.ReactNode }) {
  return (
    <div>
      <dt className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">{l}</dt>
      <dd className="mt-1 text-sm font-extrabold text-foreground">{children}</dd>
    </div>
  );
}
