...
export const create = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const item = await inventoryService.create(req.user.businessId, req.user.userId, req.body);
    res.status(201).json({
      success: true,
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const item = await inventoryService.update(req.user.businessId, req.user.userId, req.params.id, req.body);
    res.status(200).json({
      success: true,
