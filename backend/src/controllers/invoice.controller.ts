...
export const share = async (req: Request, res: Response, next: NextFunction) => { try { res.json({ success: true, message: "Invoice public link created and share recorded.", data: await service.share(bid(req), req.params.id, req.user!.userId, req.body?.phone) }); } catch (error) { next(error); } };
export const getSharesByCustomer = async (req: Request, res: Response, next: NextFunction) => { try { res.json({ success: true, data: await service.getSharesByCustomer(bid(req), req.params.customerId) }); } catch (error) { next(error); } };
